...
class RefundDishReport extends Component {
	constructor(props) {
		super(props);
		this.state={
			dishlist:[],
	...
	loadifno(str){
        ...
        let pomisetable = api.get('/statistics/statistic_cancel.json',{ params:recordobj});
		pomisetable.then((data)=>{
    ...
    exportExcel(){
    	...
    	window.location.href = baseURI + '/statistics/statistic_cancel_toexcel.json?' + str;
    }
    // 销量排序
    orderBySellAmount(){
    	const data = this.props.form.getFieldsValue();	
				...
				<Row type="flex"  className="cash-controls dishdatehh">
					...
					<div className="bill-table dailydish">
						<Table 
							...
							columns={columns} 
							pagination={false} 
							dataSource={this.state.dishlist.statisticcancel}
							scroll={{ y: 488 }}
						/>
					</div>
					<SticRefresh 
...
RefundDishReport = Form.create()(RefundDishReport);

export default RefundDishReport;
